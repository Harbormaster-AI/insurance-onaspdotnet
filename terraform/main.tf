...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "a58a166f-0bdd-4493-bef3-85f410b68cc0"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.2006052177589268"
...
resource "aws_db_subnet_group" "default" {
  name       = "insuranceonaspdotnet-db-subnet"
...
#  name        = "insuranceonaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application insuranceonaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for insuranceonaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "insuranceonaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "insuranceonaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_insuranceonaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "insuranceonaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/insurance-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/insuranceonaspdotnet theharbormaster/insurance-onaspdotnet:latest"
    ]
  }
}
